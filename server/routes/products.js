...
router.post("/checkout",async (req,res)=>{
  ...
  console.log(bcrypt.hashSync(secretkey,10))
  if(await bcrypt.compare(secretkey,wallet.secretkey)){
    const updatedWallet=await walletModel.findOneAndUpdate({username:fromUsername},{balance:wallet.balance-Number(amount)})
...
res.render("checkout2",{reference:reference,orderno:orderno,email:req.session.email,amount:Number(amount)-80,total:amount,address:address,phone:phone,day:day,month:month,year:year})
    ...
    alert("Wrong pin!");
    res.redirect("/checkout/"+req.session.email);
  }
